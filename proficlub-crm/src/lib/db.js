...
export async function updateEmployeeSalesStats() {
  try {
    ...
    for (const s of sales) {
      if (s.crm_savdo_vakili) byName[s.crm_savdo_vakili] = (byName[s.crm_savdo_vakili]||0) + (s.summa||0)
      if (s.crm_menejer) byName[s.crm_menejer] = (byName[s.crm_menejer]||0) + (s.summa||0)
    }
